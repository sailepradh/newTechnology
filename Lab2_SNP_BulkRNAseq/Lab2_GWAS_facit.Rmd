...
Genome-wide association (GWA) studies scan an entire species genome for association between up to millions of SNPs and a given trait of interest. Notably, the trait of interest can be virtually any sort of phenotype ascribed to the population, be it qualitative (e.g. disease status) or quantitative (e.g. height). Essentially, given m SNPs and n samples or individuals, a GWAS will fit m independent univariate linear models, each based on n samples, using the genotype of each SNP as predictor of the trait of interest. The significance of association (P-value) in each of the m tests is determined from the coefficient estimate \beta  of the corresponding SNP (technically speaking, the significance of association is P(\beta|H_0: \beta=0) ). It's necessary to adjust the resulting P-values using multiple hypothesis testing methods such as Bonferroni, Benjamini-Hochberg or false discovery rate (FDR).
...
Q1: Explain the result - What are we actually looking at? How many dimensions are there really in the data? 
...
Lets now perform our GWAS study. In the information we have 282 different lipids, lets choose to make the cholesterol levels our target of interest. 
...
For visualization of our results, we will use a "Manhattan plot". Note that these p-values are B-H corrected for multiple testing.  
...
Q3: How would the plot look if no BH correction had been made?
...
Q4: What is your interpretation of the Q-Q plot?
...
Q5: Is there any variant that are within a gene? Can you find any potential biological association of interest with regards to cholesterol? 
